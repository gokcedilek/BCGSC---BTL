...
time_df <- time_orig %>%
  mutate(
    ...
    stop = cumsum(Minutes),
    start = stop - Minutes)
Minutes<-as.numeric(time_df$minutes)
 ...
 time_df$step<-factor(time_df$step, levels=time_df$step)
  ggplot(time_df, aes(time_df$Minutes, time_df$step)) +
    ...
    geom_text(size=2.5, aes(x=(time_df$start + time_df$stop)/2, y=time_df$step), hjust=0.5, vjust=-1, label=paste(round(time_df$time, digits = 2), " min", sep=""))
  dev.off()
