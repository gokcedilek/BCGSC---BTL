...
using MxtoCount = tsl::robin_map<Mx, std::atomic<unsigned>>;
std::mutex m1;
std::mutex m2;
std::mutex m3;
std::mutex m4;
...
static void countMxs(unsigned thread_id, const BxtoMxs& bxtomxs, unsigned num_threads, MxtoCount& counts) {
	...
	for (unsigned index = thread_id; index < bxtomxs.size(); index+=num_threads) {
		...
		{
			std::lock_guard<std::mutex> lock(m1);//129
			const auto& mxs = it->second;
			for (const auto& mx : mxs) {
				if (counts.find(mx) == counts.end()) { //readlock
					counts[mx] = 0; //write lock (read write mutex)
					counts.insert(std::make_pair(mx, 0));
				} //else {
					++counts[mx]; //atomic incr (function) -- 
				//}
			}
